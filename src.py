...
def def_full_file_reader():
    file_foods = open('documents'+navigator_symbol+'list_foods.txt', 'r')
    for i in file_foods:
        list_foods.append(str(i.strip()))
    file_foods.close()

    file_drinks = open('documents'+navigator_symbol+'list_drinks.txt', 'r')
    for i in file_drinks:
        list_drinks.append(str(i.strip()))
    file_drinks.close()

    file_services = open('documents'+navigator_symbol+'list_services.txt', 'r')
    ...
    while i <= (len(list_services) - 1):
        if '$' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('$') - 1]) + ' ' * (20 - (
                list_services[i].index('$') - 1)) + str(list_services[i][list_services[i].index('$'):])
        i += 1
...
def def_file_sorter():
    ...
    while i < len(list_foods):
        list_item_price[i] = float(
            list_foods[i][int(list_foods[i].index("$") + 2):])
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i]
                                        [int(list_drinks[i].index("$") + 2):])
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i]
                                        [int(list_services[i].index("$") + 2):])
        i += 1
...
def def_report():
    while True:
        print("*" * 33 + "REPORT" + "*" * 33 + "\n")
        file_report = open('documents'+navigator_symbol +
                           'report.txt', 'r').read()
        print(file_report)
        print("\n(M) MAIN MENU          (E) EXIT\n" + "_" * 72)
        input_1 = str(input("Please Select Your Operation: ")).upper()
...
def def_payment():
    while True:
        ...
        if (input_1 == 'P'):
            ...
            file_report = open('documents'+navigator_symbol+'report.txt', 'a')
